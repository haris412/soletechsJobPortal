...
export class JobsListComponent implements OnInit {
  ...
  Valuechange(event:any){
    if(event?.target?.value?.length >= 1){
      this.jobsList = this.jobsListCopy.filter((job:Job)=> job.description.toLowerCase().includes(event?.target.value.toLowerCase()));
      this.sharedService.jobs = this.sharedService.DeepCopyObject(this.jobsListCopy.filter((job:Job)=> job.description.toLowerCase().includes(event?.target.value.toLowerCase())));
    }else{
      this.jobsList = this.jobsListCopy;
